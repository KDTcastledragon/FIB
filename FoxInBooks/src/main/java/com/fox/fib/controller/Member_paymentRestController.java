...
public class Member_paymentRestController {
	...
	public ResponseEntity<?> memberpaymentinsert(@RequestBody HashMap<String, Object> payment_formData, Member_payment entity,
		User_couponId dto) throws IOException {
		...
		try {
			...
			for (int i = 0; i < paymentDetailList.size(); i++) {
				...
				member_payment_detailService.insertPaymentDetail(payment_code, userId, product_code, proamount,
					(String) payment_formData.get("payment_date"), Integer.parseInt((String) payment_formData.get("origin_price")),
					Integer.parseInt((String) payment_formData.get("final_price")), (String) payment_formData.get("delivery_state"),
					(String) payment_formData.get("arrive_date"), productOne.getProtype(), productOne.getDomestic(), productOne.getTitle(),
					productOne.getImage(), productOne.getPrice());
