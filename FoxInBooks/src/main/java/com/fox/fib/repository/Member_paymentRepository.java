...
public interface Member_paymentRepository extends JpaRepository<Member_payment, Long> {
	...
	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "update member_payment set payment_cancel = 2 where member_payment_code = :member_payment_code")
	int convertCancel(@Param("member_payment_code") Long member_payment_code);
